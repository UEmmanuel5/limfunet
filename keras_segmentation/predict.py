...
def evaluate(model=None, inp_images=None, annotations=None,
    ...
    # === Added for Pixel Accuracy and Mean Accuracy ===
    total_correct_pixels = 0
    total_pixels = 0
    correct_pixels_per_class = np.zeros(model.n_classes)
    ...
    for inp, ann in tqdm(zip(inp_images, annotations)):
        ...
        total_correct_pixels += np.sum(pr == gt)  # For Pixel Accuracy
        total_pixels += len(gt)

        for cl_i in range(model.n_classes):
            correct_pixels_per_class[cl_i] += np.sum((pr == cl_i) * (gt == cl_i))  # For Mean Accuracy
    ...
    # === Existing metrics ===
    cl_wise_score = tp / (tp + fp + fn + 1e-9)
    n_pixels_norm = n_pixels / np.sum(n_pixels)
    frequency_weighted_IU = np.sum(cl_wise_score * n_pixels_norm)
    mean_IU = np.mean(cl_wise_score)

    # === New metrics computed ===
    pixel_accuracy = total_correct_pixels / (total_pixels + 1e-9)
    mean_accuracy = np.mean(correct_pixels_per_class / (n_pixels + 1e-9))

    return {
        "pixel_accuracy": pixel_accuracy,                     # ? Added
        "mean_accuracy": mean_accuracy,                       # ? Added
        "mean_IU": mean_IU,                                   # ? Already present
        "frequency_weighted_IU": frequency_weighted_IU,       # ? Already present
        "class_wise_IU": cl_wise_score                        # ? Already present
    }